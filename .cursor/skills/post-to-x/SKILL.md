...
Chrome profile directory can be overridden via the `BAOYU_CHROME_PROFILE_DIR` or `X_BROWSER_PROFILE_DIR` environment variable. Chrome binary path can be overridden via `X_BROWSER_CHROME_PATH`.
...
Custom configurations via environment variables. See **Preferences** section for available options.
